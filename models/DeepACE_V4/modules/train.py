...
def main(config, config_path):
    ...
    model = DeepACE( 
        ...
        num_conformer_blocks=deepace_params['num_conformer_blocks'],
        num_heads=deepace_params['num_heads'],
        ff_hidden_dim=deepace_params['ff_hidden_dim'],
        conv_kernel_size=deepace_params['conv_kernel_size']
    ).to(device)
    ...
    model_save_dir = os.path.join('../../../results/DeepACE_VT', f"run_{timestamp}")
