...
def kl_divergence(mean_q, logvar_q, mean_p, logvar_p):
    # Convert log variance to variance
    var_q = torch.exp(logvar_q)
    var_p = torch.exp(logvar_p)

    # Compute KL divergence
    kl = 0.5 * ( (var_q / var_p) + ((mean_p - mean_q) ** 2) / var_p - 1 + (logvar_p - logvar_q) )
    return kl.mean()

class LossFunctionSelector:
    def __init__(self):
        self.loss_functions = {
            "huber": self._get_huber_loss,
    ...
    def _get_huber_loss(self, delta=0.3):
