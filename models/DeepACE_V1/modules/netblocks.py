...
class DED(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.N = in_channels
        self.n_electrodes = out_channels

        # Define layers
        self.envdet_c1 = torch.nn.Conv1d(in_channels=self.N, out_channels=self.N//2, kernel_size=3)
        self.prelu_c1 = torch.nn.ReLU()
        
        self.envdet_c2 = torch.nn.Conv1d(in_channels=self.N // 2, out_channels=self.n_electrodes, kernel_size=6)
        self.prelu_c2 = torch.nn.ReLU()

    def forward(self, inputs):

        padding_c1 = self.envdet_c1.kernel_size[0] - 1  # For kernel_size=3, padding_c1=2
        x = torch.nn.functional.pad(inputs, (padding_c1, 0))  # Pad left
        x = self.envdet_c1(x)
        x = self.prelu_c1(x)


        padding_c2 = self.envdet_c2.kernel_size[0] - 1  # For kernel_size=6, padding_c2=5
        x = torch.nn.functional.pad(x, (padding_c2, 0))  # Pad left
        x = self.envdet_c2(x)
        x = self.prelu_c2(x)
