...
class OLSATestWindow(QMainWindow):
    """OLSA test window implementation."""
    
    # Signal to notify when window is closing
    window_closing = pyqtSignal()
    
    def __init__(self, eng, name, surname, id_val, map_data=None, map_number=None):
        super().__init__()
        self.eng = eng
        self.name = name
        ...
        # Set audio folder
        self.audio_folder = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'olsa', 'audio')
    ...
    def initUI(self):
        ...
        # Subject and Map Information
        side_info = "N/A"
        if self.map_data:
            try:
                # Get side from the map data
                map_str = str(self.map_data)
                if "left" in map_str.lower():
                    side_info = "Left"
                elif "right" in map_str.lower():
                    side_info = "Right"
            except:
                side_info = "N/A"
        
        infoLabel = QLabel(f"Subject ID: {self.id_val} | Map: {self.map_number} | Side: {side_info}")
    ...
    def refreshAudioFiles(self):
        ...
        try:
            ...
            if self.audioFileCombo.count() == 0:
                self.statusLabel.setText("No OLSA files found in folder. Using defaults.")
                self.statusLabel.setStyleSheet("color: #ffc107;")
            else:
                self.statusLabel.setText(f"Found {self.audioFileCombo.count()} OLSA files.")
                self.statusLabel.setStyleSheet("color: #28a745;")
                
        except Exception as e:
            default_files = ["olsa_set1.wav", "olsa_set2.wav", "olsa_practice.wav"]
            self.audioFileCombo.clear()
    ...
    def closeEvent(self, event):
        ...
        print("OLSA test window closeEvent called")
